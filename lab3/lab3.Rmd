...
###4)
Vi börjar med bayers sats
$$ f(\theta \mid y) = f(y \mid \theta)*f(\theta) / f(y) $$
#Eftersom att vi ska härleda posteriorn för mu så byter vi theta mot mu
$$ f(\mu \mid y) = f(y \mid \mu)*f(\mu) / f(y) $$
#Vi kan sedan förkorta bort de faktorer som ej inehåller mu
$$ f(\mu \mid y) = f(y \mid \mu)*f(\mu) $$
#Vi ersätter = med ∝ eftersom att de är propertionella mot varandra
$$ f(\mu \mid y) ∝ f(y \mid \mu)*f(\mu) $$
#Där har vi svaret!
###5)
```{r }
...
##3.2.1
###1)
Vi väljer att sätta prior till (1,1) eftersom att vi ej vet någon data sedan tidigare. Så alla fall är lika sannolika
```{r }
    a = 1
    b = 1
    interval <- seq(0, 1, 0.01)
    prior_pa <- dbeta(x = interval, shape1 = alpha, shape2 = b)
    prior_pb <- dbeta(x = interval, shape1 = beta, shape2 = b)
